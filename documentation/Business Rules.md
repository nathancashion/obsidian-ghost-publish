...
- **BR-PROC-7**: Academic citations (`services/render-citations.ts`) are opt-in **per preset** (`citationsEnabled`) with global config (`settings.citations`: pandoc path, bibliography, CSL style). Processing runs `pandoc --citeproc` markdown → markdown, **after** wikilink/image/marker resolution and **before** the HTML build, so a note-class CSL style emits regular markdown footnotes that flow through BR-PROC-5/6. Only a **bracketed** citation (`[@key]`) triggers the pandoc run (bare `@key` risks false positives on handles); the post's bibliography section is suppressed (`suppress-bibliography`) because note-style footnotes carry the full citation. The pandoc writer disables `fenced_divs` / `bracketed_spans` so citeproc wrappers can't leak `:::` syntax through `marked`. Config paths resolve absolute > `~/` > vault-relative. Failures throw with actionable messages and surface as failed notes (BR-SYNC-5).
