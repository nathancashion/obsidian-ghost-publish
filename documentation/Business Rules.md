...
- **BR-PROC-7**: Academic citations (`services/render-citations.ts`) are opt-in **per preset** (`citationsEnabled`) with global config (`settings.citations`: pandoc path, bibliography, CSL style). Processing runs `pandoc --citeproc` markdown → markdown, **after** wikilink/image/marker resolution and **before** the HTML build, so a note-class CSL style emits regular markdown footnotes that flow through BR-PROC-5/6. The pandoc run triggers on a **bracketed** citation (`[@key]`) OR a **bare** citation (`@key` not preceded by a word character — emails don't match; user decision 2026-08-18: bare keys are common in their prose, and `@handle` mentions in citations-enabled notes are escaped as `\@handle`; the escaped form also triggers the run, since pandoc is what strips the escape). The post's bibliography section is suppressed (`suppress-bibliography`) because note-style footnotes carry the full citation. The pandoc writer disables `fenced_divs` / `bracketed_spans` so citeproc wrappers can't leak `:::` syntax through `marked`. Config paths resolve absolute > `~/` > vault-relative. A bare pandoc command is resolved via `resolvePandocBinary` against PATH **plus** `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin` — macOS GUI apps get the minimal launchd PATH, so a plain `execFile('pandoc')` would ENOENT even with pandoc installed. Failures throw with actionable messages and surface as failed notes (BR-SYNC-5); non-fatal citeproc stderr (e.g. unresolved keys) is logged as a warning.
